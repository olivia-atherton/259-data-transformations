...
---
`r chunk_reveal("types", break_type = "auto", widths = c(2, 1))`
...
- Comparisons between values that result in a logical output (TRUE/FALSE)
...
---
`r chunk_reveal("logical", break_type = "auto", widths = c(2, 1))`
...
---
`r chunk_reveal("arrange", break_type = "rotate", float = "none", widths = c(5, 1))`
...
arrange(ds, -height, mass) #ROTATE
arrange(ds, eye_color, hair_color) #ROTATE
```

---
`r chunk_reveal("filter", break_type = "rotate", float = "none", widths = c(5, 1))`
...
---
`r chunk_reveal("select", break_type = "rotate", float = "none", widths = c(5, 1))`
...
select(ds, is.numeric) #ROTATE
select(ds, is.character) #ROTATE
```

---
`r chunk_reveal("noassignment", break_type = "auto", widths = c(1, 1))`
...
---
`r chunk_reveal("reassignment", break_type = "auto", left_assign = T, widths = c(1, 1))`

```{r reassignment, include = FALSE}
ds <- select(ds, name, height, eye_color)
ds <- arrange(ds, height, eye_color)
ds <- filter(ds, height < 100)
```
...
ds_sorted_filtered <- filter(ds_sorted, height < 100)
ds_sorted_filtered
```
